    ...
    <div v-if="cardLabels.length" class="board-card__labels">
      <span
        ...
        :style="{
          ...
          '--label-content-surface': 'var(--content-board-label-card-surface)',
          '--label-content-text': 'var(--content-board-label-text)',
        }"
      >{{ label.title }}</span>
    </div>
...
.board-card__label { padding:2px 7px; border-radius:999px; background:color-mix(in srgb, var(--label-color) 32%, var(--label-content-surface)); color:var(--label-content-text); font-size:10px; font-weight:650; }
...
.board-card--overdue .board-card__due { color:var(--ui-danger-foreground); background:var(--ui-danger-soft); }
